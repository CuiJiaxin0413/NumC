...
static PyObject *Matrix61c_add(Matrix61c* self, PyObject* args) {
    /* TODO: YOUR CODE HERE */
    // TypeError if either a or b is not of type numc.Matrix.
...
static PyObject *Matrix61c_pow(Matrix61c *self, PyObject *pow, PyObject *optional) {
    ...
    int ret = pow_matrix(new_mat, self->mat, int_pow);
    if (ret == 1) {
        PyErr_SetString(PyExc_ValueError, "pow is negative!");
        return NULL;
    } else if (ret == 2) {
        PyErr_SetString(PyExc_ValueError, "matrix is not a square matrix!");
        return NULL;
    }
...
static PyObject *Matrix61c_set_value(Matrix61c *self, PyObject* args) {
    ...
    if (PyArg_UnpackTuple(args, "args", 1, 3, &i, &j, &val)) {
        ...
        if (i && j && val && PyLong_Check(i) && PyLong_Check(j) && (PyLong_Check(val) || PyFloat_Check(val))) {
            if (PyLong_AsLong(i) >= self->mat->rows || PyLong_AsLong(j) >= self->mat->cols || PyLong_AsLong(i) < 0 || PyLong_AsLong(j) < 0) {
                PyErr_SetString(PyExc_IndexError, "Index out of range");
                return NULL;
            }
            if (PyLong_Check(val)) {
                set(self->mat, PyLong_AsLong(i), PyLong_AsLong(j), (double) PyLong_AsLong(val));
    ...
    if (PyArg_UnpackTuple(args, "args", 1, 2, &i, &j)) {
        ...
        if (i && j && PyLong_Check(i) && PyLong_Check(j)) {
            if (PyLong_AsLong(i) >= self->mat->rows || PyLong_AsLong(j) >= self->mat->cols || PyLong_AsLong(i) < 0 || PyLong_AsLong(j) < 0) {
                ...
                return PyFloat_FromDouble(get(self->mat, PyLong_AsLong(i), PyLong_AsLong(j)));
            }
